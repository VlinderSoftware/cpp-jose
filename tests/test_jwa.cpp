...
TEST_F(JWATest, HS256SignAndVerify) {
    ...
    std::vector<unsigned char> signature = JWA::sign(
...
TEST_F(JWATest, RSA_OAEP_EncryptDecrypt) {
    ...
    std::vector<unsigned char> encrypted = JWA::encryptKey(
    ...
    std::vector<unsigned char> decrypted = JWA::decryptKey(
        JWA::KeyEncryptionAlgorithm::RSA_OAEP,
        key,
        encrypted
    );
...
TEST_F(JWATest, A128GCM_EncryptDecrypt) {
    ...
    auto [ciphertext, tag] = JWA::encryptContent(
